...
class DigitalTwin:
    def __init__(self):
        ...
        self.l = 0.28   # Length of the pendulum (m)
        self.c_air = 0.17  # Air friction coefficient
        self.c_c = 0.46   # Coulomb friction coefficient
        self.a_m = 1500 # Motor acceleration force tranfer coefficient
    ...
    def butterworth_filter(self, data, cutoff_freq, fs, order=5):
        nyquist_freq = 0.5 * fs
    ...
    def load_filtered_data(self, name):
        self.df = pd.read_csv('{}.csv'.format(name))
        # Apply the filter to the loaded theta data
        cutoff_frequency = 10  # Or whatever value is deemed appropriate
        fs = 1 / (np.diff(self.df['time'].values).mean())  # Sampling frequency
        self.df['filtered_theta'] = self.butterworth_filter(self.df['theta'].values, cutoff_frequency, fs)
        print("Filtered recording is loaded")

    def recorded_step(self, i):
        # Use the filtered theta
        a = self.df["time"].iloc[i]
        b = self.df["filtered_theta"].iloc[i]  # Use the filtered theta here
        c = self.df["x_pivot"].iloc[i]  
        return a, b, c
    ...
    def get_theta_double_dot(self, theta, theta_dot):
        ...
        """
        # Calculate the sign of theta_dot for the Coulomb friction component
        sign_theta_dot = -1 if theta_dot < 0 else (1 if theta_dot > 0 else 0)

        # Calculate the angular acceleration of the pendulum
        theta_double_dot = (-self.g * np.sin(theta) - self.c_air * theta_dot - self.c_c * sign_theta_dot + self.a_m * self.currentmotor_acceleration) / self.l
